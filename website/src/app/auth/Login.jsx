...
export default function Login() {
  ...
  useEffect(() => {
    console.log('Modal state changed:', loginStatus);
  }, [loginStatus]);
  return (
    <section
      className={` ${
      ...
      >
        ...
        <div className="text-center">
          <h3 className="text-[22px] font-semibold mb-1">
            {compStatus ? "Welcome back!" : "Welcome !"}
          </h3>
          <p className="text-[14px] font-semibold">
            {compStatus ? "Log in" : "Sign up"} to enjoy your perks
          </p>
        </div>
...
function LoginBox() {
  ...
  let handleLogin=(event)=>{
    ...
    axios.post(websiteBaseUrl+"auth/login",loginData)
    .then((res)=>{
      if(res.data.status){
  ...
  return (
    <form className="flex flex-col gap-3 py-6" onSubmit={handleLogin}>
      ...
      <button type="submit" className="p-3.5 mt-2 bg-black text-white font-semibold">
        Log In
      </button>
    </form>
  );
...
function SignUpBox({
  ...
  let createUser = (event) => {
    event.preventDefault();
    axios.post(websiteBaseUrl + "auth/generate-otp", signUpData).then((res) => {
      if (res.data.status) {
  ...
  let verifyOTP = (event) => {
    event.preventDefault();
    axios
      .post(websiteBaseUrl + "auth/check-otp-register", signUpData)
      .then((res) => {
        if (res.data.status) {
